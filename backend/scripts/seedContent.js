/**
 * Standalone script to seed content into the database
 * Run with: node backend/scripts/seedContent.js
 */
...
async function seedContent() {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/movieSync');
    console.log('✅ Connected to MongoDB');

    // Delete existing content
    const deleted = await Content.deleteMany({});
    console.log(`🗑️  Deleted ${deleted.deletedCount} existing content items`);

    // Insert new content
    const inserted = await Content.insertMany(sampleContent);
    console.log(`✅ Successfully inserted ${inserted.length} content items`);

    // Verify
    const count = await Content.countDocuments();
    console.log(`📊 Total content in database: ${count}`);
